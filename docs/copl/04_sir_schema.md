...
### 2.1 Workspace (Không gian làm việc gốc)
...
### 2.3 Module (Khối thành phần lõi)
...
├── size_bytes: Option<U32>         (* tính toán tự động nếu kiểu có kích thước tĩnh - statically sized *)
└── derives: List<String>
...
│   ├── deadlock_states: Set<String> (* các trạng thái bị khóa cứng (deadlock), không có đường chuyển tiếp *)
...
│   ├── cycles: List<List<String>>  (* phụ thuộc vòng tròn - circular dependencies *)
...
Các API dưới đây cung cấp cơ chế để bất kỳ công cụ bên ngoài nào đều có thể truy vấn xuất dữ liệu:
